...
class GeometryParser:
    ...
    def parse_text(self, text):
        ...
        # CHUẨN HÓA VĂN BẢN: Xóa bỏ các dấu chấm câu và ký tự lạ để tránh lệch từ khóa
        ...
        # Trường hợp 2: Hình lăng trụ đứng ABC.A'B'C' (NÂNG CẤP ĐA BỘ LỌC TỪ KHÓA THÔNG MINH CHỐNG LỖI TỰ ĐỘNG GÕ TIẾNG VIỆT)
        if "lăng trụ" in raw_text or "lang tru" in raw_text or "abc" in raw_text or "a'b'c'" in raw_text or "a’b’c’" in raw_text:
            self.points['A'] = [0.0, 0.0, 0.0]
            self.points['B'] = [4.0, 0.0, 0.0]
        ...
        for match in line_matches:
            p1, p2 = match
            # Hỗ trợ tự động nhận diện nối điểm có dấu gạch của tiếng Việt dấu sắc
            p1_clean = p1.replace("’", "'")
            p2_clean = p2.replace("’", "'")
